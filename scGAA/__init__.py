...
from . import pre, train, scGAA_model
...
def train(adata, project=None, pre_weights='', label_name='label',max_g=399,max_gs=399,mask_ratio =0.015, n_unannotated = 1,batch_size=8, embed_dim=48,depth=2,num_heads=4,lr=0.001, epochs= 10, lrf=0.01):
    ...
    max_g
        The largest number of genes in a gene set.
    max_gs
        Maximum number of gene sets.
    mask_ratio
    ...
    embed_dim
        Dimensions of gene set embeddings.
    depth
...
def pre(adata,model_weight_path,project,laten=False,save_att = 'X_att', save_lantent = 'X_lat',n_step=10000,cutoff=0.1,n_unannotated = 1,batch_size=50,embed_dim=48,depth=2,num_heads=4):
    ...
    cutoff
        Unknown cutoff.
    n_unannotated
        The number of fully connected gene sets to be added. Should be the same as train.
    batch_size
        The number of cells for training in one epoch.
    embed_dim
        The dimension of gene sets embedding. Should be the same as train.
    depth
    ...
    adata
        ...
        adata.var['node_index'] : Gene set of each colume
    """
